...
export class OffboardingTaskService {
  constructor(
    ...
    private readonly repo: Repository<OffboardingTask>,
  ) {}
  ...
  async findOne(id: string): Promise<OffboardingTask> {
    const entity = await this.repo.findOne({ where: { id } });
    if (!entity) throw new NotFoundException(`Offboarding task ${id} not found`);
    return entity;
  }

  async create(
    data: Partial<OffboardingTask> & { companyId?: string },
  ): Promise<OffboardingTask> {
