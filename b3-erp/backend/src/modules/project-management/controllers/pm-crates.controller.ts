import { Controller, Get, Post, Put, Delete, Body, Param, Query } from '@nestjs/common';
import { PmCratesService } from '../services/pm-crates.service';
import { PmCrateEntity } from '../entities/pm-crate.entity';

@Controller('project-management/crates')
export class PmCratesController {
  constructor(private readonly service: PmCratesService) {}

  @Get()
  findAll(@Query('companyId') companyId?: string, @Query('status') status?: string) {
    return this.service.findAll(companyId || 'default', status);
  }

  @Get(':id')
  findOne(@Param('id') id: string) {
    return this.service.findOne(id);
  }

  @Post()
  create(@Body() body: Partial<PmCrateEntity>) {
    return this.service.create(body);
  }

  @Put(':id')
  update(@Param('id') id: string, @Body() body: Partial<PmCrateEntity>) {
    return this.service.update(id, body);
  }

  @Delete(':id')
  remove(@Param('id') id: string) {
    return this.service.remove(id);
  }
}
