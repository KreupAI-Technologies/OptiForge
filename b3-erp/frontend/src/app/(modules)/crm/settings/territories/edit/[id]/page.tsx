...
export default function TerritoryEditPage({ params }: { params: { id: string } }) {
  const router = useRouter();
  const [form, setForm] = useState<TerritoryForm>({
    name: '',
    country: '',
    state: '',
    city: '',
    ...
    <div className="w-full h-full px-3 py-2 space-y-3">
      ...
      {!isLoading && !loadError && !notFound && (
        <form onSubmit={handleSubmit} className="bg-white rounded-lg border border-gray-200 p-6 space-y-4 max-w-3xl">
          <h1 className="text-xl font-bold text-gray-900">Edit Territory</h1>
