...
export default function EquipmentRateHistoryPage({ params }: { params: { id: string } }) {
  const router = useRouter()
  const [rate, setRate] = useState<EquipmentRate | null>(null)
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setNotFound(false)
      try {
        const detail = await estimationResourceRateService.findEquipmentRateById(COMPANY_ID, params.id)
        if (cancelled) return
        if (!detail) {
          setNotFound(true)
          return
        ...
        setRate(detail)
        try {
          const rows = await estimationResourceRateService.getEquipmentRateHistory(COMPANY_ID, params.id)
          if (!cancelled) setHistory(rows)
        } catch {
          if (!cancelled) setHistory([])
        }
      } catch (err) {
        if (!cancelled) setLoadError(err instanceof Error ? err.message : 'Failed to load equipment rate')
      } finally {
    ...
    <div className="w-full h-full px-4 py-2">
      <div className="mb-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600 mt-1">Rate-change history for this equipment rate</p>
          </div>
        </div>
        {rate && (
          <button
            onClick={() => router.push('/estimation/rates/equipment')}
      ...
      {isLoading && (
        <div className="mb-3 flex items-center gap-2 rounded-lg border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-700">
          ...
          Loading equipment rate history…
        </div>
      )}
      {loadError && !isLoading && (
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5 mb-3">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div>
                ...
                <p className="text-sm font-bold text-blue-600">{fmtCurrency(rate.hourlyRate)}</p>
              </div>
              <div>
                <p className="text-xs text-gray-500 uppercase">Daily Rate</p>
                <p className="text-sm font-medium text-gray-900">{fmtCurrency(rate.dailyRate)}</p>
              </div>
              <div>
                <p className="text-xs text-gray-500 uppercase">Weekly Rate</p>
                <p className="text-sm font-medium text-gray-900">{fmtCurrency(rate.weeklyRate)}</p>
              </div>
              <div>
                <p className="text-xs text-gray-500 uppercase">Monthly Rate</p>
                <p className="text-sm font-medium text-gray-900">{fmtCurrency(rate.monthlyRate)}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200">
            ...
            {history.length === 0 ? (
              <div className="px-5 py-8 text-center text-sm text-gray-500">
                ...
                No rate-change history recorded for this equipment rate.
              </div>
            ) : (
