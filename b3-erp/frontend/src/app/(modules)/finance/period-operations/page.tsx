...
import { Calendar, CalendarCheck, RefreshCw, ArrowRight } from 'lucide-react';
    ...
    (async () => {
      try {
        ...
        const [periods, years] = await Promise.all([
          FinanceService.getFinancialPeriods().catch(() => []),
          FinanceService.getFinancialYears().catch(() => []),
        ]);
        if (!cancelled) {
          setCounts({
            'Period Close': (periods || []).length,
            'Year-End Close': (years || []).length,
          });
        }
      } catch (e) {
        if (!cancelled) setError(e instanceof Error ? e.message : 'Failed to load period operations data');
      } finally {
          ...
          <div className="w-full space-y-3">
            {loading && (
              <div className="flex items-center gap-2 rounded-lg border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-700">
                <RefreshCw className="h-4 w-4 animate-spin" /> Loading period operations…
              </div>
            )}
            {error && !loading && (
