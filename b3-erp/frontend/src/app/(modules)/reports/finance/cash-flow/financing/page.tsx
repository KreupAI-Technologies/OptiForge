...
function FinancingActivitiesContent() {
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            try {
                ...
                const mapped = raw.map((r: any) => ({ id: r.paymentNumber ?? r.id, paymentId: r.id, date: r.paymentDate ?? '', description: r.notes ?? r.partyName ?? '', category: r.paymentType ?? 'Financing', amount: Number(r.amount ?? 0) }));
    ...
    return (
        <ReportDetailPage
            ...
            onBack={() => router.back()}
            onExport={() => exportToCsv('cash-flow-financing', transactions)}
        >
