...
export default function Page() {
  ...
  const [detailReturn, setDetailReturn] = useState<ESIReturn | null>(null);

  const handleDownloadReturn = (r: ESIReturn) => {
    const url = (r as any).documentUrl || (r as any).fileUrl || (r as any).url;
    if (url) { window.open(url, '_blank'); return; }
    const escape = (v: string) => `"${(String(v) ?? '').replace(/"/g, '""')}"`;
    const headers = ['Period', 'Establishment', 'ESI Code', 'Branch', 'Due Date', 'Filing Date', 'Status', 'Total Employees', 'Covered Employees', 'Gross Wages', 'Employee Contribution', 'Employer Contribution', 'Total Contribution', 'Challan Number'];
    const row = [r.returnPeriod, r.establishment, r.esiCode, r.branch, r.dueDate, r.filingDate ?? '', r.status, String(r.totalEmployees), String(r.coveredEmployees), String(r.grossWages), String(r.employeeContribution), String(r.employerContribution), String(r.totalContribution), r.challanNumber ?? ''];
    const csv = [headers.map(escape).join(','), row.map(escape).join(',')].join('\n');
    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
    const blobUrl = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = blobUrl;
    a.download = `esi-return-${r.returnPeriod || r.id}.csv`;
            ...
            return (
              <div key={esiReturn.id} className="bg-white rounded-lg shadow-sm border border-gray-200 p-3">
                ...
                <div className="flex gap-2">
                  <button onClick={() => setDetailReturn(esiReturn)} className="px-4 py-2 bg-pink-600 text-white rounded-lg hover:bg-pink-700 text-sm font-medium flex items-center gap-2">
                    <FileText className="h-4 w-4" />
                    View Details
                  </button>
                  <button onClick={() => handleDownloadReturn(esiReturn)} className="px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 text-sm font-medium text-gray-700 flex items-center gap-2">
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-lg w-full max-w-lg max-h-[90vh] overflow-y-auto p-5">
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-lg font-bold text-gray-900">ESI Return — {detailReturn.returnPeriod || detailReturn.returnMonth}</h2>
            ...
            <div className="grid grid-cols-2 gap-3 text-sm mb-3">
              ...
              <div><span className="text-gray-500">ESI Code:</span> <span className="font-medium text-gray-900">{detailReturn.esiCode}</span></div>
              <div><span className="text-gray-500">Branch:</span> <span className="font-medium text-gray-900">{detailReturn.branch}</span></div>
              ...
              <div><span className="text-gray-500">Employee Contribution:</span> <span className="font-medium text-gray-900">₹{detailReturn.employeeContribution.toLocaleString()}</span></div>
              <div><span className="text-gray-500">Employer Contribution:</span> <span className="font-medium text-gray-900">₹{detailReturn.employerContribution.toLocaleString()}</span></div>
              <div><span className="text-gray-500">Total Contribution:</span> <span className="font-medium text-gray-900">₹{detailReturn.totalContribution.toLocaleString()}</span></div>
