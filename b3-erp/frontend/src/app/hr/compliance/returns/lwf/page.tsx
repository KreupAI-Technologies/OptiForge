...
export default function Page() {
  ...
  const [detailReturn, setDetailReturn] = useState<LWFReturn | null>(null);

  const handleDownloadReturn = (r: LWFReturn) => {
    const url = (r as any).documentUrl || (r as any).fileUrl || (r as any).url;
    if (url) { window.open(url, '_blank'); return; }
    const escape = (v: string) => `"${(String(v) ?? '').replace(/"/g, '""')}"`;
    const headers = ['Return Month', 'State', 'Registration Number', 'Due Date', 'Filing Date', 'Status', 'Total Employees', 'Covered Employees', 'Employee Contribution', 'Employer Contribution', 'Total Contribution', 'Challan Number'];
    const row = [r.returnMonth, r.state, r.registrationNumber, r.dueDate, r.filingDate ?? '', r.status, String(r.totalEmployees), String(r.coveredEmployees), String(r.employeeContribution), String(r.employerContribution), String(r.totalContribution), r.challanNumber ?? ''];
    const csv = [headers.map(escape).join(','), row.map(escape).join(',')].join('\n');
    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
    const blobUrl = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = blobUrl;
    a.download = `lwf-return-${r.returnMonth || r.id}.csv`;
            ...
            return (
              <div key={lwfReturn.id} className="bg-white rounded-lg shadow-sm border border-gray-200 p-3">
                ...
                <div className="flex gap-2">
                  <button onClick={() => setDetailReturn(lwfReturn)} className="px-4 py-2 bg-cyan-600 text-white rounded-lg hover:bg-cyan-700 text-sm font-medium flex items-center gap-2">
                    <FileText className="h-4 w-4" />
                    View Details
                  </button>
                  <button onClick={() => handleDownloadReturn(lwfReturn)} className="px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 text-sm font-medium text-gray-700 flex items-center gap-2">
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-lg w-full max-w-lg max-h-[90vh] overflow-y-auto p-5">
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-lg font-bold text-gray-900">LWF Return — {detailReturn.returnMonth}</h2>
            ...
            <div className="grid grid-cols-2 gap-3 text-sm mb-3">
              ...
              <div><span className="text-gray-500">Registration Number:</span> <span className="font-medium text-gray-900">{detailReturn.registrationNumber}</span></div>
