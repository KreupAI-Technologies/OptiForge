...
import { Receipt, Download, Eye, FileText, AlertCircle, Send } from 'lucide-react';
...
export default function ExpensePolicyPage() {
  ...
  const load = async () => {
    ...
    setLoadError(null);
    try {
      const { data } = await DocumentManagementService.getHRPolicies({ policyCategory: PolicyCategory.EXPENSE_POLICY });
            ...
            <div key={topic.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-200 hover:border-orange-300 hover:bg-orange-50 transition-colors">
              ...
              <div className="flex gap-2 ml-4">
                <button
                  ...
                  className="px-4 py-2 text-orange-600 hover:bg-orange-100 rounded-lg font-medium text-sm flex items-center gap-2"
                >
                ...
                {topic.status === 'draft' && (
                  <button
                    ...
                    className="px-4 py-2 text-white bg-orange-600 hover:bg-orange-700 rounded-lg font-medium text-sm flex items-center gap-2"
                  >
