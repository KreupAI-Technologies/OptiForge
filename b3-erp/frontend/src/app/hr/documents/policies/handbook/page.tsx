...
import { Book, Download, Eye, FileText, CheckCircle, AlertCircle, Send } from 'lucide-react';
...
export default function EmployeeHandbookPage() {
  ...
  const load = async () => {
    ...
    setLoadError(null);
    try {
      const { data } = await DocumentManagementService.getHRPolicies({ policyCategory: PolicyCategory.EMPLOYEE_HANDBOOK });
      const mapped: HandbookSection[] = data.map((policy) => ({
        id: policy.id,
  ...
  const primaryPolicy = handbookSections[0];
            ...
            <div key={section.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-200 hover:border-blue-300 hover:bg-blue-50 transition-colors">
              <div className="flex-1">
                ...
                <p className="text-sm text-gray-600 mt-1">
                  {section.description}
                  {section.lastUpdated && ` • Last updated: ${new Date(section.lastUpdated).toLocaleDateString('en-IN')}`}
                </p>
              </div>
              <div className="flex gap-2 ml-4">
                <button
                  onClick={() => handleDownload(section.fileUrl)}
