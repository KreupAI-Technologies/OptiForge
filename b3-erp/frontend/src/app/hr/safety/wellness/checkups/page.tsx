...
export default function WellnessCheckupsPage() {
  ...
  const [form, setForm] = useState({ employeeName: '', title: 'Annual Health Screening' });

  // derived from fetched checkupLog
  const completedCount = checkupLog.filter((c) => c.status === 'Completed').length;
  const checkupStats = {
    completionRate: checkupLog.length
      ? Math.round((completedCount / checkupLog.length) * 1000) / 10
      : 0,
    pendingCheckups: checkupLog.filter(
      (c) => c.status === 'Scheduled' || c.status === 'Overdue',
    ).length,
    highRiskFlags: checkupLog.filter(
      (c) => c.compliance && c.compliance !== 'Compliant' && c.compliance !== 'On Track',
    ).length,
    remindersSent: checkupLog.filter((c) => c.status === 'Scheduled').length,
  };
  ...
  const handleScheduleCheckup = async () => {
    ...
    try {
      await HrSafetyService.createWellness({
        ...
        title: form.title || 'Health Screening',
        scheduledDate: new Date().toISOString().slice(0, 10),
        status: 'Scheduled',
      });
      setShowCreate(false);
      setForm({ employeeName: '', title: 'Annual Health Screening' });
      await load();
    } catch (err) {
      setLoadError(err instanceof Error ? err.message : 'Failed to schedule checkup');
    } finally {
      setIsSaving(false);
    }
  };
      ...
      {showCreate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
            <h2 className="mb-4 text-lg font-bold text-gray-900">Schedule Checkup</h2>
            <div className="space-y-3">
              ...
              <div>
                <label className="mb-1 block text-sm font-medium text-gray-700">Assessment Type</label>
                <input
                  className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm"
                  value={form.title}
