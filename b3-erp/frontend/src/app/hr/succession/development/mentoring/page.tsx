...
import { Users, TrendingUp, Calendar, Target, MessageCircle, CheckCircle, Pencil } from 'lucide-react';
...
export default function Page() {
  ...
  const [editRow, setEditRow] = useState<MentoringPair | null>(null);
  const [editForm, setEditForm] = useState<Partial<MentoringPair>>({});
  ...
  const handleSave = async () => {
    ...
    setSaveError(null);
    try {
      const { id, ...rest } = { ...editRow, ...editForm } as MentoringPair;
          ...
          <div key={pair.id} className="bg-white rounded-lg shadow-sm border border-gray-200 p-3">
            ...
            <div className="pt-4 border-t border-gray-200">
              ...
              <div className="mt-2 flex gap-2">
                <button
                  onClick={() => { setEditRow(pair); setEditForm({ status: pair.status, progress: pair.progress, meetingFrequency: pair.meetingFrequency, programType: pair.programType }); setSaveError(null); }}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-lg bg-white shadow-xl">
            <div className="border-b border-gray-200 px-5 py-3">
              <h2 className="text-lg font-bold text-gray-900">Edit Mentoring Pair</h2>
              <p className="text-sm text-gray-600">{editRow.mentor.name} → {editRow.mentee.name}</p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 px-5 py-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Status</label>
                <select value={editForm.status ?? 'active'} onChange={(e) => setEditForm(f => ({ ...f, status: e.target.value as MentoringPair['status'] }))} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500">
                  <option value="active">Active</option>
                  <option value="completed">Completed</option>
                  <option value="on_hold">On Hold</option>
                  <option value="discontinued">Discontinued</option>
              ...
              <div>
                ...
                <select value={editForm.programType ?? 'succession'} onChange={(e) => setEditForm(f => ({ ...f, programType: e.target.value as MentoringPair['programType'] }))} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500">
                  <option value="succession">Succession</option>
                  <option value="leadership">Leadership</option>
                  <option value="technical">Technical</option>
                  <option value="career_development">Career Development</option>
              ...
              <div>
                ...
                <select value={editForm.meetingFrequency ?? 'monthly'} onChange={(e) => setEditForm(f => ({ ...f, meetingFrequency: e.target.value as MentoringPair['meetingFrequency'] }))} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500">
